...
const coins = [
  { name: "ETH", fullName: "Ethereum", price: "$66 390.25", change: "+2.35%", volume: "1.2 B",icon: Eth},
  { name: "BTC", fullName: "Bitcoin", price: "$2 197.77", change: "+3.03%", volume: "610.3 B", icon: Btc },
  { name: "SOL", fullName: "Solana", price: "$144.85", change: "-1.16%", volume: "432.9 B", icon: Sol },
  { name: "XRP", fullName: "Ripple", price: "$0.624", change: "-0.56%", volume: "211.6 B", icon: Group },
  { name: "LTC", fullName: "Litecoin", price: "$6.13", change: "+2.35%", volume: "93.1 B", icon: Ltc },
  { name: "DOGE", fullName: "Dogecoin", price: "$1.250", change: "-0.41%", volume: "173.8 B", icon: Doge },
];

export default function CryptoTables() {

   const [activeTab, setActiveTab] = useState("popular");
   const filteredCoins = activeTab === "popular"
  ? coins 
  ...
  return (
    <section className="crypto-table hightlightbox">
        <div className="crypto-table__container container">
          <div className="wrap">
              <h2 className="section-title heading h2">Track trends, pick top coins</h2>
                ...
                <table className="crypto-table__table">
                  <thead className="">
                    <tr className="crypto-table__thead">
                      ...
                      <th className="crypto-table__trade">Trading history for 24 h.</th>
...
<path d="M14.3333 15.5L9.08333 10.25C8.66667 10.5833 8.1875 10.8472 7.64583 11.0417C7.10417 11.2361 6.52778 11.3333 5.91667 11.3333C4.40278 11.3333 3.12153 10.809 2.07292 9.76042C1.02431 8.71181 0.5 7.43056 0.5 5.91667C0.5 4.40278 1.02431 3.12153 2.07292 2.07292C3.12153 1.02431 4.40278 0.5 5.91667 0.5C7.43056 0.5 8.71181 1.02431 9.76042 2.07292C10.809 3.12153 11.3333 4.40278 11.3333 5.91667C11.3333 6.52778 11.2361 7.10417 11.0417 7.64583C10.8472 8.1875 10.5833 8.66667 10.25 9.08333L15.5 14.3333L14.3333 15.5ZM5.91667 9.66667C6.95833 9.66667 7.84375 9.30208 8.57292 8.57292C9.30208 7.84375 9.66667 6.95833 9.66667 5.91667C9.66667 4.875 9.30208 3.98958 8.57292 3.26042C7.84375 2.53125 6.95833 2.16667 5.91667 2.16667C4.875 2.16667 3.98958 2.53125 3.26042 3.26042C2.53125 3.98958 2.16667 4.875 2.16667 5.91667C2.16667 6.95833 2.53125 7.84375 3.26042 8.57292C3.98958 9.30208 4.875 9.66667 5.91667 9.66667Z" fill="white"/>
                      ...
                      <tr key={idx} className="crypto-table__row">
                        <td className="crypto-table__column">
                          <div className="crypto-table__icons">
                            <img src={coin.icon} width={50} height={50} alt={coin.fullName} className="crypto-table__icon" />
                          </div>
                          <div className="crypto-table__names">
                            <div className="crypto-table__short">{coin.name}</div>
                            <div className="crypto-table__full">{coin.fullName}</div>
                          </div>
                        </td>
                        <td className="">{coin.price}</td>
                        <td className="">{coin.volume}</td>
                        <td className={coin.change.startsWith('+') ? "crypto-table__change--positive" : "crypto-table__change--negative"}>{coin.change}</td>
                        <td><CustomButton text="Trade" className="crypto-table__trade button-1"></CustomButton></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="crypto-table__btns">
                  <button className={`crypto-table__btn ${activeTab === "popular" ? "active" : ""}`}
        onClick={() => setActiveTab("popular")}>
                  ...
                  <button  className={`crypto-table__btn ${activeTab === "growing" ? "active" : ""}`}
    ...
    <tr key={idx} className="crypto-table__row">
      ...
      <td>{coin.volume}</td>
      <td className={coin.change.startsWith('+') ? "crypto-table__change--positive" : "crypto-table__change--negative"}>{coin.change}</td>
      <td><CustomButton text="Trade" className="crypto-table__trade button-1" /></td>
    </tr>
  ))}
                  </tbody>
                </table>
